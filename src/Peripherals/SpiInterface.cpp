//
// Created by david on 1-11-18.
//

#include <Peripherals/SpiInterface.h>

#include <linux/spi/spidev.h>
#include <sys/ioctl.h>
#include <fcntl.h>
#include <cstdint>
#include <unistd.h>
#include <iostream>
#include <cstring>

#define WRITE                   0x80
#define READ                    0x00

const uint8_t BitReverseTable[256] =
        {
                
        };

Peripherals::SpiInterface::SpiInterface(const std::string &path)
        : m_buffer{0}, m_speed(SPI_SPEED), m_bitsPerWord(SPI_BITS_PER_WORD) {
    m_fd = open(path.c_str(), O_RDWR);

    if (m_fd <= 0)
        throw "Unable to open SPI";

    if (ioctl(m_fd, SPI_IOC_WR_BITS_PER_WORD, &m_bitsPerWord) < 0)
        throw "Unable to set bits per word";
//
    if (ioctl(m_fd, SPI_IOC_WR_MAX_SPEED_HZ, &m_speed) < 0)
        throw "Unable to set SPI m_speed";
}

Peripherals::SpiInterface::SpiInterface(const std::string &path, uint8_t mode)
        : m_buffer{0}, m_speed(SPI_SPEED), m_bitsPerWord(SPI_BITS_PER_WORD) {
    m_fd = open(path.c_str(), O_RDWR);

    if (m_fd <= 0)
        throw "Unable to open SPI";

    if (ioctl(m_fd, SPI_IOC_WR_MODE, &mode) < 0)
        throw "Unable to set SPI mode";

    if (ioctl(m_fd, SPI_IOC_RD_MODE, &mode) < 0)
        throw "Unable to set SPI mode";

    if (ioctl(m_fd, SPI_IOC_WR_BITS_PER_WORD, &m_bitsPerWord) < 0)
        throw "Unable to set bits per word";

    if (ioctl(m_fd, SPI_IOC_RD_BITS_PER_WORD, &m_bitsPerWord) < 0)
        throw "Unable to set bits per word";

    if (ioctl(m_fd, SPI_IOC_WR_MAX_SPEED_HZ, &m_speed) < 0)
        throw "Unable to set SPI m_speed";

    if (ioctl(m_fd, SPI_IOC_RD_MAX_SPEED_HZ, &m_speed) < 0)
        throw "Unable to set SPI m_speed";
}

Peripherals::SpiInterface::~SpiInterface() {
    close(m_fd);
}

void Peripherals::SpiInterface::WriteRaw(uint8_t value) {
    std::lock_guard<std::mutex> lock(m_sending);
    m_buffer[SPI_WRITE_REGISTER_BUFFER] = BitReverseTable[value];

    write(m_fd, m_buffer + SPI_WRITE_REGISTER_BUFFER, 1);
}

void Peripherals::SpiInterface::WriteRaw(uint8_t *values, uint8_t size) {
    std::lock_guard<std::mutex> lock(m_sending);
    for (int i = 0; i < size; ++i) {
        values[i] = BitReverseTable[values[i]];
    }

    write(m_fd, values, size);
}

uint8_t Peripherals::SpiInterface::ReadRaw() {
    std::lock_guard<std::mutex> lock(m_sending);
    read(m_fd, m_buffer + SPI_READ_VALUE_BUFFER, 1);

    return BitReverseTable[m_buffer[SPI_READ_VALUE_BUFFER]];
}

void Peripherals::SpiInterface::ReadRaw(uint8_t *buffer, uint8_t size) {
    std::lock_guard<std::mutex> lock(m_sending);
    read(m_fd, buffer, size);

    for (int i = 0; i < size; ++i) {
        buffer[i] = BitReverseTable[buffer[i]];
    }
}

void Peripherals::SpiInterface::ReadWriteRaw(uint8_t *txBuffer, uint8_t *rxBuffer, uint8_t size) {
    struct spi_ioc_transfer xfer = {0};

    xfer.tx_buf = (unsigned long) txBuffer;
    xfer.rx_buf = (unsigned long) rxBuffer;
    xfer.len = size;

    std::lock_guard<std::mutex> lock(m_sending);
    ioctl(m_fd, SPI_IOC_MESSAGE(1), &xfer);
}
